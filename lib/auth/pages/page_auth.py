...
class PageAuth(Settings):
    ...
    def log_in(self, login: str = None, password: str or int = None):
        ...
        self.__login_input.should(be.visible).set_value(login or self.login)
        self.__password_input.should(be.visible).set_value(password or self.password)
    ...
    def __set_registration_data(self):
        ...
        self.__first_name_input.with_(Config(timeout=10)).should(be.visible).set_value(first_name)
